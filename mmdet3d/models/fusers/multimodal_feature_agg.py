...
from ..modules.groupmixattention import Aggregator

__all__ = ["multimodal_feature_agg"]

@FUSERS.register_module()
class multimodal_feature_agg(nn.Module):
    def __init__(self, num_sweeps=4, img_dims=64, lidar_dims=128, radar_dims=64, embed_dims=256,
                 num_layers=6, num_heads=4, num_modalities=3, bev_shape=(128, 128)):
        super(multimodal_feature_agg, self).__init__()
        ...
        self.norm_lidar = build_norm_layer(dict(type='LN'), lidar_dims)[1]
        self.norm_radar = build_norm_layer(dict(type='LN'), radar_dims)[1]

        self.input_proj = nn.Linear(img_dims + lidar_dims+ radar_dims  , self.embed_dims) #
        self.agga = Aggregator(self.embed_dims, seg=5)

        self.bev_h, self.bev_w = bev_shape  #bev feature map==128*128
        ...
        for _ in range(num_layers):
            self.attn_layers.append(
                DeformableCrossAttention(
                    ...
                    lidar_dims=self.lidar_dims,
                    radar_dims=self.radar_dims,
                    embed_dims=self.embed_dims,
    ...
    @auto_fp16(apply_to=('feat_img' , 'feat_lidar', 'feat_radar')) #
    def forward(self, feat_img, feat_lidar, feat_radar): #
        ...
        feat_img = self.norm_img(feat_img.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()  ##这个维度的改变还待商榷
        feat_lidar = self.norm_lidar(feat_lidar.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
        feat_radar = self.norm_radar(feat_radar.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()

        feat_flatten = []
        spatial_shapes = []
        for feat in [feat_img, feat_lidar, feat_radar]: #
            _, _, h, w = feat.shape
            spatial_shape = (h, w)
        ...
        bev_queries = self.agga(bev_queries, size)
        #
        bev_mask = torch.zeros((bs, self.bev_h, self.bev_w),
                               device=bev_queries.device).to(feat_img.dtype)
        bev_pos = self.positional_encoding(bev_mask).to(feat_img.dtype)# bs dim_embeds bev_h bev_w
        bev_pos = bev_pos.flatten(2).permute(0, 2, 1).contiguous()

        feat_img = feat_flatten[0]
        feat_lidar = feat_flatten[1]
        feat_radar = feat_flatten[2] #[2]
