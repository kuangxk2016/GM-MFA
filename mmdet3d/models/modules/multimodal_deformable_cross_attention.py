...
from .groupmixattention import Aggregator
...
class DeformableCrossAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.value_proj_lidar = nn.Linear(lidar_dims, embed_dims)
        self.value_proj_radar = nn.Linear(radar_dims, embed_dims)
    ...
    def init_weights(self):
        ...
        xavier_init(self.value_proj_lidar, distribution='uniform', bias=0.)
        xavier_init(self.value_proj_radar, distribution='uniform', bias=0.)
    ...
    def forward(self,
        ...
        _, num_value_lidar, _ = value_lidar.shape
        _, num_value_radar, _ = value_radar.shape


        value_img = self.value_proj_img(value_img)
        value_img = self.agga(value_img,size)
        value_img = value_img.reshape(bs, num_value_img, self.num_heads, -1)
        value_lidar = self.value_proj_lidar(value_lidar)
        value_lidar = self.agga(value_lidar, size)
        value_lidar = value_lidar.reshape(bs, num_value_lidar, self.num_heads, -1)
        value_radar = self.value_proj_radar(value_radar)
        value_radar = self.agga(value_radar, size)
        value_radar = value_radar.reshape(bs, num_value_radar, self.num_heads, -1)
        value = torch.cat([value_img, value_lidar, value_radar], dim=1)
